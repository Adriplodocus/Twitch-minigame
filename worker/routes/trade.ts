...
trade.get("/users/:username", requireAuth, async (c) => {
  ...
  const cards = await c.env.DB.prepare(
    `SELECT c.id, c.name, c.rarity, c.image_path AS imagePath, c.sort_order AS sortOrder, c.generation AS generation, COALESCE(uc.quantity, 0) - COALESCE(uc.reserved, 0) AS quantity
     FROM cards c
